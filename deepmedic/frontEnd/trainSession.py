...
class TrainSession(Session):
    ...
    def run_session(self, *args):
        ...
        with graphTf.as_default():
            ...
            # Create the savers
            saver_all = tf.compat.v1.train.Saver(max_to_keep=0)  # Will be used during training for saving everything.
        ...
        with tf.compat.v1.Session(graph=graphTf,
            ...
            if file_to_load_params_from is not None:  # Load params
                ...
                if not reset_trainer:
                    ...
                    trainer.get_param_updates_wrt_task()
                    self._log.print3("Trainer parameters were updated wrt. task.")
                else:
